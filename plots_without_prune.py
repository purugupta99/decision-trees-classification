...
# # print(tree)
# # print(count_number_of_non_leaf_nodes(tree))


# # In[37]:


# pruned_tree = post_prune(df_validate, 200, 2, tree)


# # In[16]:


# df_train['predicted'] = df_train.apply(classify, axis=1, args=(tree,'No'))
# df_test['predicted'] = df_test.apply(classify, axis=1, args=(tree,'No'))

# print('Train set accuracy: ' + str( sum(df_train['salary']==df_train['predicted'] ) / (len(df_train.index)) ))
# print('Test set accuracy: ' + str( sum(df_test['salary']==df_test['predicted'] ) / (len(df_test.index)) ))
...
# print('Test set accuracy on pruned tree: ' + str( sum(df_test['salary']==df_test['predicted'] ) / (len(df_test.index)) ))
...
for p in percent:
    ...
    node_num.append(count_number_of_non_leaf_nodes(tree))
...
plt.plot(percent, [baseline_accuracy]*4, label='Baseline Accuracy')

plt.title('Accuracy vs Train Set Percentage')
...
plt.title('Number of Nodes vs Train Set Percentage')
